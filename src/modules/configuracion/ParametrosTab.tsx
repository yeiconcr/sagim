...
export function ParametrosTab() {
  ...
  const handleGuardar = async () => {
    if (!nombreGimnasio.trim()) {
      error('Falta nombre', 'El nombre del gimnasio es requerido.');
      return;
    }
    setGuardando(true);
    try {
      ...
      <div className="max-w-4xl space-y-6 pb-6">
        <div className="flex justify-between items-center bg-white p-4 rounded-lg border shadow-sm sticky top-0 z-10">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Personaliza la apariencia y el comportamiento de SAGIM.
            </p>
          </div>
          {!editando ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <CardContent className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2 mt-2">
                <label className="block text-xs font-medium text-slate-700 mb-1">
                  Logo del Gimnasio (Para reportes y PDFs)
                </label>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold text-slate-600">
                Facturación e Impresión POS
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <FormField
                label="Resolución DIAN / Texto Legal"
                htmlFor="p-res"
                description="Se imprimirá en la cabecera de los recibos."
              >
                <textarea
                  id="p-res"
              ...
              <FormField
                ...
                htmlFor="p-msg"
                description="Se imprimirá al final del recibo POS."
              >
                <textarea
                  id="p-msg"
                  value={mensajeRecibo}
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold text-slate-600">
                Reglas y Operatividad
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <FormField
                  label="Días para inactivar cliente"
                  htmlFor="p-inact"
                  description="Después de vencidos."
                >
                  <Input
                    id="p-inact"
                    type="number"
              ...
              <div className="pt-2 border-t flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Si se desactiva, la tienda bloqueará ventas si el artículo llegó a 0.
                  </p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  ...
                  />
                  <div
                    className={`w-11 h-6 bg-slate-200 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all ${permitirSinStock === 1 && !!editando ? 'bg-blue-600' : ''} ${!editando ? 'opacity-50' : ''}`}
                  ></div>
                </label>
              </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {editando && (
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => setColorPrimario('#1e40af')}
