...
export function FormaPagoTab() {
  ...
  const abrirNuevo = () => { reset({ plazo_dias: 0 }); setEditando(null); setMostrarForm(true); };
  const abrirEditar = (f: FormaPago) => { reset({ detalle: f.detalle, plazo_dias: f.plazo_dias }); setEditando(f); setMostrarForm(true); };
  const cerrar = () => { setMostrarForm(false); setEditando(null); reset(); };

  const onSubmit = async (data: FormValues) => {
    try {
      if (editando) {
        await updateFormaPago(editando.id, data.detalle, data.plazo_dias);
        success("Forma de pago actualizada");
      } else {
        await createFormaPago(data.detalle, data.plazo_dias);
    ...
    <div className="flex flex-col gap-4 h-full">
      {mostrarForm && (
        <div className="flex items-end gap-3 p-4 bg-white rounded-lg border flex-wrap">
          ...
          <FormField label="Plazo en días" error={errors.plazo_dias?.message} htmlFor="fp-pla" className="w-36">
            <Input id="fp-pla" type="number" min="0" {...register("plazo_dias")} className={cn(errors.plazo_dias && "border-destructive")} />
          </FormField>
          <div className="flex gap-2">
            <Button type="button" size="sm" onClick={handleSubmit(onSubmit)}>
              <Save className="w-3.5 h-3.5 mr-1" />{editando ? "Guardar" : "Crear"}
            </Button>
            <Button type="button" variant="outline" size="sm" onClick={cerrar}><X className="w-3.5 h-3.5" /></Button>
          </div>
        </div>
      )}
