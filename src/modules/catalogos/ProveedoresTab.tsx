...
export function ProveedoresTab() {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      if (!editando) {
        const nitExiste = await proveedorNitExiste(data.nit);
        if (nitExiste) {
          setFormError('nit', { message: 'Este NIT ya está registrado' });
          return;
        }
      }

      if (editando) {
        await updateProveedor(editando.id, {
          nombre: data.nombre,
      ...
      cell: ({ row }) => (
        <div>
          ...
          {row.original.contacto && (
            <p className="text-xs text-slate-400">{row.original.contacto}</p>
          )}
        </div>
      ),
    },
    ...
    <div className="flex flex-col h-full gap-3">
      ...
      <div className="flex justify-between items-center flex-shrink-0">
        <p className="text-sm text-slate-500">
          {proveedores.filter((p) => p.estado === 'A').length} proveedores activos
        </p>
        {!mostrarForm && (
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
                ...
                >
                  <Input
                    ...
                    {...register('nit')}
                    disabled={!!editando}
                    className={cn(errors.nit && 'border-destructive')}
                  />
                </FormField>
                <FormField
