...
import {
  ...
  createEspecialidad,
  updateEspecialidad,
  toggleEspecialidadEstado,
  especialidadNombreExiste,
...
export function EspecialidadesTab() {
  ...
  const abrirEditar = (e: Especialidad) => {
    reset({ nombre: e.nombre });
    setEditando(e);
    setMostrarForm(true);
  };
  const cerrar = () => {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      const existe = await especialidadNombreExiste(data.nombre.trim(), editando?.id);
      if (existe) {
        error('Duplicado', 'Ya existe una especialidad con este nombre');
        return;
      }

      if (editando) {
        await updateEspecialidad(editando.id, data.nombre.trim());
        success('Especialidad actualizada');
      } else {
        await createEspecialidad(data.nombre.trim());
    ...
    <div className="flex flex-col h-full gap-3">
      ...
      {mostrarForm && (
        <div className="flex items-end gap-3 p-4 bg-white rounded-lg border flex-shrink-0">
          <FormField
            label={editando ? 'Editar especialidad' : 'Nueva especialidad'}
      ...
      <div className="flex-1 min-h-0 flex flex-col">
        <DataTable
          ...
          emptyMessage="No hay especialidades."
