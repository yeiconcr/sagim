...
export function PagosInstructoresModule() {
  ...
  const handleGuardar = async () => {
    if (!idInstructor) {
      error('Falta instructor', 'Seleccione un instructor.');
      return;
    }
    ...
    try {
      ...
      success(
        'Pago registrado',
        `Pago de ${formatCurrency(val)} registrado. Egreso creado en caja.`
      );
      setVista('lista');
      cargar();
    } catch (err) {
