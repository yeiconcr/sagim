...
import { Plus, Truck, ArrowLeft, Plus as PlusIcon, Trash2, Save, X, AlertCircle } from "lucide-react";
...
export function ComprasModule() {
  ...
  const cargarFormulario = useCallback(async () => {
    const [prov, art, fps] = await Promise.all([getProveedores(), getInventario({ pageSize: 500 }), getFormasPago(true)]);
    setProveedores(prov);
    setArticulos(art.data);
    setFormasPago(fps);
    if (fps.length > 0) { setIdFormaPago(String(fps[0].id)); setPlazo(fps[0].plazo_dias); }
  }, []);
  ...
  const abrirNuevo = () => {
    setItems([]);
    setIdProveedor(""); setFecha(today()); setNroDocumento(""); setObservaciones(""); setArticuloSel("");
    cargarFormulario();
    setVista("nuevo");
  };
  ...
  const handleGuardar = async () => {
    ...
    try {
      const nroCompra = await guardarCompra({
        ...
        plazo,
        items: items.map((i) => ({ codigo: i.codigo, detalle: i.detalle, cantidad: i.cantidad, punitario: i.punitario, total: i.total })),
        total: totalCompra,
        observaciones,
    ...
    return (
      ...
      <div className="p-6 max-w-5xl mx-auto min-h-full flex flex-col gap-4">
        ...
        <Card>
          <CardContent className="p-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <FormField label="Fecha" htmlFor="com-fecha">
              <Input id="com-fecha" type="date" value={fecha} onChange={(e) => setFecha(e.target.value)} />
            </FormField>
            <FormField label="N° Documento" htmlFor="com-doc">
              <Input id="com-doc" value={nroDocumento} onChange={(e) => setNroDocumento(e.target.value)} placeholder="Factura del proveedor" />
            </FormField>
            <FormField label="Forma de pago" className="col-span-2">
              <Select value={idFormaPago} onValueChange={(v) => { setIdFormaPago(v); setPlazo(formasPago.find(f => String(f.id) === v)?.plazo_dias ?? 0); }}>
                <SelectTrigger className="h-9"><SelectValue /></SelectTrigger>
                <SelectContent>
                  {formasPago.map((fp) => <SelectItem key={fp.id} value={String(fp.id)}>{fp.detalle}{fp.plazo_dias > 0 ? ` (${fp.plazo_dias} días)` : ""}</SelectItem>)}
                </SelectContent>
              </Select>
            </FormField>
            <FormField label="Observaciones" htmlFor="com-obs" className="col-span-2">
              <Input id="com-obs" value={observaciones} onChange={(e) => setObservaciones(e.target.value)} placeholder="Observaciones..." />
            </FormField>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold text-slate-600">Artículos comprados</CardTitle></CardHeader>
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 border-b">
                  <tr>
                    ...
                    <th className="text-right px-2 py-1.5 text-xs font-semibold text-slate-500 uppercase w-28">P. Compra</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {items.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-1"><Button type="button" variant="ghost" size="icon" className="h-7 w-7" onClick={() => setItems((p) => p.filter((i) => i.id !== item.id))}><Trash2 className="w-3.5 h-3.5 text-red-400" /></Button></td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="border-t">
                    <td colSpan={3} className="px-2 py-2 text-right font-bold text-slate-700">TOTAL COMPRA:</td>
                    <td className="px-2 py-2 text-right font-black text-lg text-slate-900 tabular-nums">{formatCurrency(totalCompra)}</td>
                    <td></td>
                  </tr>
                </tfoot>
              </table>
            ) : (
              <div className="text-center py-8 text-slate-400 text-sm"><AlertCircle className="w-8 h-8 mx-auto mb-2 opacity-40" />Agregue artículos a la compra</div>
            )}
          </CardContent>
        </Card>
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button onClick={handleGuardar} disabled={guardando || !idProveedor || items.length === 0} className="min-w-[140px]">
