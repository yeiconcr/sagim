...
// =============================================
const RecepcionModule = lazy(() =>
  import('@/modules/recepcion/RecepcionModule').then((m) => ({ default: m.RecepcionModule }))
);
const ClientesModule = lazy(() =>
  import('@/modules/clientes/ClientesModule').then((m) => ({ default: m.ClientesModule }))
);
const CatalogosModule = lazy(() =>
  import('@/modules/catalogos/CatalogosModule').then((m) => ({ default: m.CatalogosModule }))
);
const InventarioModule = lazy(() =>
  import('@/modules/inventario/InventarioModule').then((m) => ({ default: m.InventarioModule }))
);
const VentasGymModule = lazy(() =>
  import('@/modules/ventas-gym/VentasGymModule').then((m) => ({ default: m.VentasGymModule }))
);
const VentasTiendaModule = lazy(() =>
  import('@/modules/ventas-tienda/VentasTiendaModule').then((m) => ({
    default: m.VentasTiendaModule,
  }))
);
const ComprasModule = lazy(() =>
  import('@/modules/compras/ComprasModule').then((m) => ({ default: m.ComprasModule }))
);
const CajaModule = lazy(() =>
  import('@/modules/caja/CajaModule').then((m) => ({ default: m.CajaModule }))
);
const CarteraModule = lazy(() =>
  import('@/modules/cartera/CarteraModule').then((m) => ({ default: m.CarteraModule }))
);
const PagosInstructoresModule = lazy(() =>
  import('@/modules/pagos-instructores/PagosInstructoresModule').then((m) => ({
    default: m.PagosInstructoresModule,
  }))
);
const ProcesosModule = lazy(() =>
  import('@/modules/procesos/ProcesosModule').then((m) => ({ default: m.ProcesosModule }))
);
const ReportesModule = lazy(() =>
  import('@/modules/reportes/ReportesModule').then((m) => ({ default: m.ReportesModule }))
);
const ConfiguracionModule = lazy(() =>
  import('@/modules/configuracion/ConfiguracionModule').then((m) => ({
    default: m.ConfiguracionModule,
  }))
);
...
const ModuleRenderer = memo(function ModuleRenderer({ modulo }: { modulo: ModuloActivo }) {
  switch (modulo) {
    case 'recepcion':
      return (
        <Suspense fallback={<PageLoading text="Cargando..." />}>
          <RecepcionModule />
        </Suspense>
      );
    case 'clientes':
      ...
      >
        ...
        {!sidebarCollapsed && (
          <div className="min-w-0 flex-1">
            <p className="font-black text-sm text-white line-clamp-2 leading-tight break-words">
              {nombreGimnasio}
            </p>
          ...
          return (
            <div key={item.id}>
              {item.separator && (
                <div
                  className={cn(
                    'border-t border-slate-700/50 my-1',
...
export function AppShell() {
  ...
  useEffect(() => {
    ...
    getVersion()
      .then((v) => {
        if (!cancelled) setAppVersion('v' + v);
      })
      .catch(console.error);
    return () => {
      cancelled = true;
