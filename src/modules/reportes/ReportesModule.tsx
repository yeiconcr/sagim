...
import {
  ...
  getDetReciboPorNro,
  getFacturasTienda,
  getDetFacturaTiendaPorNro,
} from '@/db/queries/ventas';
...
import {
  ...
  InventarioPDF,
  KardexPDF,
  TrazabilidadMedidasPDF,
  MovimientoCajaPDF,
  VentasDiariasPDF,
  ComprasRealizadasPDF,
  CuentasPorPagarPDF,
  EgresosPDF,
  CobrosRealizadosPDF,
  ReciboInstructorPDF,
...
const COLOR_THEMES = {
  blue: {
    ...
    text: 'text-blue-600 group-hover:text-indigo-600',
  },
  green: {
    bg: 'from-green-50 to-emerald-50 border-green-100/50',
    text: 'text-green-600 group-hover:text-emerald-600',
  },
  purple: {
    bg: 'from-purple-50 to-fuchsia-50 border-purple-100/50',
    text: 'text-purple-600 group-hover:text-fuchsia-600',
  },
  orange: {
    bg: 'from-orange-50 to-amber-50 border-orange-100/50',
    text: 'text-orange-600 group-hover:text-amber-600',
  },
  rose: {
    bg: 'from-rose-50 to-pink-50 border-rose-100/50',
    text: 'text-rose-600 group-hover:text-pink-600',
  },
  teal: {
    bg: 'from-teal-50 to-cyan-50 border-teal-100/50',
    text: 'text-teal-600 group-hover:text-cyan-600',
  },
};
...
const REPORTES: ReporteConfig[] = [
  {
    ...
    descripcion: 'Todos los clientes registrados',
    icono: Users,
    categoria: 'Clientes',
    colorTheme: 'blue',
    filtros: ['estado'],
  },
  {
    ...
    descripcion: 'Ficha personal de un cliente con medidas',
    icono: Contact,
    categoria: 'Clientes',
    colorTheme: 'blue',
    filtros: ['cedula'],
  ...
  {
    id: 'trazabilidad-medidas',
    titulo: 'Trazabilidad de Medidas',
    descripcion: 'Historial de medidas corporales por cliente',
    icono: Scale,
    categoria: 'Clientes',
    colorTheme: 'blue',
    filtros: ['cedula'],
...

  {
    ...
    descripcion: 'Stock y precios de todos los artículos',
    icono: Package,
  ...
  {
    id: 'kardex',
    titulo: 'Kárdex de Artículos',
    descripcion: 'Movimientos de un artículo por período',
    icono: BarChart2,
    categoria: 'Inventario',
    colorTheme: 'purple',
    filtros: ['codigoArticulo', 'fechaDesde', 'fechaHasta'],
  },

  {
    id: 'ventas-diarias',
    titulo: 'Ventas Diarias',
    descripcion: 'Recibos gym + facturas tienda del período',
    icono: TrendingUp,
    categoria: 'Ventas',
    colorTheme: 'green',
...

  {
    ...
    descripcion: 'Todos los movimientos financieros del período',
    icono: Wallet,
    categoria: 'Finanzas',
    colorTheme: 'teal',
    filtros: ['fechaDesde', 'fechaHasta'],
  },
  {
    id: 'egresos',
    titulo: 'Egresos',
    descripcion: 'Solo movimientos de salida del período',
    icono: TrendingDown,
    categoria: 'Finanzas',
    colorTheme: 'rose',
    filtros: ['fechaDesde', 'fechaHasta'],
  ...
  {
    id: 'cobros-realizados',
    titulo: 'Cobros Realizados',
    descripcion: 'Pagos recibidos de clientes en el período',
    icono: Coins,
    categoria: 'Finanzas',
    colorTheme: 'teal',
    filtros: ['fechaDesde', 'fechaHasta'],
  ...
  {
    id: 'compras-realizadas',
    titulo: 'Compras Realizadas',
    descripcion: 'Compras a proveedores del período',
    icono: Truck,
    categoria: 'Compras',
    colorTheme: 'orange',
    filtros: ['fechaDesde', 'fechaHasta'],
  },
  {
    ...
    descripcion: 'Saldos pendientes con proveedores',
    icono: Receipt,
    categoria: 'Compras',
    colorTheme: 'orange',
    filtros: [],
  ...
  {
    id: 'pagos-instructores',
    titulo: 'Recibo Pago Instructor',
    descripcion: 'Comprobante de pago a un instructor',
    icono: UserCheck,
...

  {
    ...
    descripcion: 'Registro de entradas al gimnasio',
    icono: Calendar,
    categoria: 'Control',
    colorTheme: 'teal',
    filtros: ['fechaDesde', 'fechaHasta'],
      ...
      switch (reporteSeleccionado.id) {
        ...
        case 'cuentas-por-pagar': {
          ...
          rows = cuentas.map((c) => [
            ...
            c.pagado,
            c.saldo,
            c.estado === 'P' ? 'PENDIENTE' : 'CANCELADA',
          ]);
          break;
        }
        case 'ventas-diarias': {
        ...
        case 'movimiento-caja': {
          ...
          rows = result.data.map((m) => {
            const ref =
              m.referencia && !isNaN(Number(m.referencia))
                ? String(m.referencia).padStart(6, '0')
                : m.referencia;
            return [
              m.fecha,
              ref,
              m.nombre_cliente || m.cedula || '—',
              m.concepto,
              m.natural === 'I' ? 'INGRESO' : 'EGRESO',
              m.valor,
            ];
          });
          break;
        ...
        case 'kardex': {
          if (!codigoArticulo.trim()) {
            error('Falta código', 'Ingrese el código del artículo.');
            setGenerando(false);
            return;
          }
          ...
          if (!art) {
            error('No encontrado', 'No existe artículo.');
            setGenerando(false);
            return;
          }
          ...
          rows = kardex.map((k) => [
            k.fecha,
            k.detalle,
            k.cantidad_in > 0 ? `+${k.cantidad_in}` : '',
            k.total_in > 0 ? k.total_in : '',
            k.cantidad_sa > 0 ? `-${k.cantidad_sa}` : '',
            k.total_sa > 0 ? k.total_sa : '',
            k.saldo,
          ]);
          break;
        }
  ...
  const generarReporte = useCallback(async () => {
    ...
    try {
      ...
      switch (reporteSeleccionado.id) {
        case 'listado-clientes': {
          ...
          doc = (
            <ListadoClientesPDF
              ...
              filtro={
                filtroEstado === 'A' ? 'Activos' : filtroEstado === 'I' ? 'Inactivos' : 'Todos'
              }
            />
          );
          break;
        ...
        case 'ficha-cliente': {
          if (!cedula.trim()) {
            error('Falta cédula', 'Ingrese la cédula del cliente.');
            setGenerando(false);
            return;
          }
          const cliente = await getClienteByCedula(cedula.trim());
          if (!cliente) {
            error('No encontrado', `No existe cliente con cédula ${cedula}`);
            setGenerando(false);
            return;
          }
        ...
        case 'kardex': {
          ...
          const art = await getArticuloByCodigo(codigoArticulo.trim());
          if (!art) {
            error('No encontrado', `No existe artículo con código ${codigoArticulo}`);
            setGenerando(false);
            return;
          }
          const kardex = await getKardexByArticulo(codigoArticulo.trim(), fechaDesde, fechaHasta);
          ...
          doc = (
            <VentasDiariasPDF
              ...
              data={[
                ...recibos.data.map((r) => ({
                  tipo: 'Recibo Gym',
                  nro_docu: String(r.nro_docu).padStart(6, '0'),
                  fecha: r.fecha,
                  nombre_cliente: r.nombre_cliente || r.cedula || '—',
                  total: r.total ?? 0,
                })),
                ...facturas.data.map((f) => ({
                  tipo: 'Factura Tienda',
                  nro_docu: String(f.nro_docu).padStart(6, '0'),
                  fecha: f.fecha,
                  nombre_cliente: f.nombre_cliente || f.cedula || '—',
                  total: f.total,
                })),
              ...
              totales={{
                'Total Recibos Gym': totalRecibos,
                'Total Facturas Tienda': totalFacturas,
                'TOTAL GENERAL': totalRecibos + totalFacturas,
              }}
            />
          );
          break;
        ...
        case 'movimiento-caja': {
          ...
          const totalI = result.data
            .filter((m) => m.natural === 'I')
            .reduce((s, m) => s + m.valor, 0);
          const totalE = result.data
            .filter((m) => m.natural === 'E')
            .reduce((s, m) => s + m.valor, 0);
          doc = (
            <MovimientoCajaPDF
              {...paramsBase}
        ...
        case 'cuentas-por-pagar': {
          ...
          doc = (
            <CuentasPorPagarPDF
              ...
              data={cuentas}
              getCell={(row: Record<string, unknown>, key: string) => {
                if (key === 'fecha_doc' || key === 'fecha_ven')
        ...
        case 'pagos-instructores': {
          ...
          doc = (
            <ReciboInstructorPDF
              ...
              data={result.data}
              getCell={(row: Record<string, unknown>, key: string) => {
                if (key === 'fecha_pag' || key === 'periodo_ini' || key === 'periodo_fin')
                  return formatDate(row[key] as string);
                if (key === 'valor') return formatCurrency(row.valor as number);
                return String(row[key] ?? '—');
              }}
        ...
        default:
          error(
            ...
            `El reporte ${reporteSeleccionado.id} no está disponible.`
          );
          setGenerando(false);
          return;
      }
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="relative z-10 flex items-start gap-3 sm:gap-4 w-full min-w-0">
                          <div
                            className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-300 border shadow-sm ${theme.bg}`}
                          >
                            <Icon
                              className={`w-5 h-5 sm:w-6 sm:h-6 transition-colors ${theme.text}`}
                            />
                          </div>
                          <div className="min-w-0 pt-0.5 sm:pt-1 flex-1">
                            <p className="font-bold text-sm text-slate-800 mb-0.5 sm:mb-1 group-hover:text-blue-700 transition-colors truncate">
                              {r.titulo}
                            </p>
                            <p className="text-xs text-slate-500 leading-snug sm:leading-relaxed line-clamp-2">
                              {r.descripcion}
                            </p>
                          </div>
                        </div>
      ...
      <div className="h-full overflow-y-auto">
        <div className="p-6 max-w-5xl mx-auto min-h-full">
            ...
            <CardContent className="p-5 space-y-4">
              {r.filtros.includes('fechaDesde') && (
                <div className="grid grid-cols-2 gap-3">
                  <FormField label="Fecha desde" htmlFor="r-fd">
                    <DatePicker
                      value={fechaDesde}
                  ...
                  <FormField label="Fecha hasta" htmlFor="r-fh">
                    <DatePicker
                      value={fechaHasta}
              ...
              {r.filtros.includes('estado') && (
                <FormField label="Estado">
                  <Select
                    value={filtroEstado}
                    onValueChange={(v) => setFiltroEstado(v as 'A' | 'I' | 'todos')}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
          ...
          <div className="flex justify-end gap-3 mt-4">
            <Button variant="outline" onClick={() => setEstado('lista')}>
