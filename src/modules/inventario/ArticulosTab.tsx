...
import {
  ...
  createArticulo,
  updateArticulo,
  toggleArticuloEstado,
  getKardexByArticulo,
  getSiguienteCodigoArticulo,
  articuloCodigoExiste,
...
export function ArticulosTab() {
  ...
  const abrirEditar = (art: Inventario) => {
    reset({
      ...
      id_proveedor: art.id_proveedor ?? undefined,
      estado: art.estado,
    });
    setEditando(art);
    setMostrarForm(true);
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      if (editando) {
        ...
        const codigoExiste = await articuloCodigoExiste(data.codigo);
        if (codigoExiste) {
          error('Código duplicado', 'Ya existe un artículo con este código.');
          return;
        }
  ...
  if (vista === 'kardex' && articuloKardex) {
    return (
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              Código: {articuloKardex.codigo} · Stock actual:{' '}
              <strong>
                {articuloKardex.stock} {articuloKardex.unidad_medida}
              </strong>
            </p>
          </div>
        </div>
        ...
        {loadingKardex ? (
          <PageLoading text="Cargando kárdex..." />
        ) : (
          <DataTable
            columns={
              [
                ...
                },
                {
                  ...
                  cell: ({ getValue }) => {
                    ...
                    return (
                      <span
                        className={cn(
                          'text-sm tabular-nums text-right block',
                          v > 0 && 'text-green-600 font-medium'
                        )}
                      >
                        {v > 0 ? `+${v}` : '—'}
                      </span>
  ...
  const columns: ColumnDef<Inventario>[] = [
    ...
    },
    {
      ...
      size: 70,
      cell: ({ row }) => (
        <span className={cn('text-sm tabular-nums', row.original.stock <= 0 && 'text-red-500')}>
          {row.original.stock}
        </span>
      ),
    },
    {
      ...
      cell: ({ getValue }) => (
        <Badge
          variant="outline"
          className="text-[10px] uppercase font-semibold text-slate-500 bg-slate-50"
        >
          {getValue<string>()}
        </Badge>
      ),
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-sm tabular-nums text-green-700">
          {formatCurrency(row.original.precio_compra * (1 + row.original.ganancia / 100))}
        </span>
      ),
    },
    {
  ...
  return (
    <div className="flex flex-col h-full gap-3">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
                ...
                >
                  <Input
                    ...
                    className={cn(errors.precio_compra && 'border-destructive')}
                  />
                </FormField>
      ...
      <div className="flex-1 min-h-0 flex flex-col">
        {loading ? (
          <PageLoading text="Cargando inventario..." />
        ) : (
          <DataTable
            columns={columns}
            data={articulos}
