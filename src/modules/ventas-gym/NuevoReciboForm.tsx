...
import { getActividades } from "@/db/queries/catalogos";
...
export function NuevoReciboForm({ cedulaInicial, onGuardar, onCancelar }: Props) {
  ...
  const handleGuardar = async () => {
    if (!cliente) { error("Falta cliente", "Debe seleccionar un cliente."); return; }
    if (items.length === 0) { error("Sin ítems", "Agregue al menos un servicio."); return; }
    if (cliente.estado === "I") { error("Cliente inactivo", "El cliente está inactivo. Active el cliente antes de registrar el pago."); return; }
    ...
    try {
      ...
      success("Recibo guardado", `Recibo N° ${String(nroDocu).padStart(6, "0")} registrado exitosamente.`);
      onGuardar();
    } catch (err) {
    ...
    <div className="p-6 max-w-5xl mx-auto min-h-full flex flex-col gap-4">
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-semibold text-slate-600">Datos del cliente</CardTitle>
        </CardHeader>
        <CardContent className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <FormField label="Fecha del recibo" htmlFor="rec-fecha">
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-semibold text-slate-600">Servicios / Actividades</CardTitle>
        </CardHeader>
                ...
                <SelectContent>
                  {actividades.map((a) => (
                    <SelectItem key={a.codigo} value={a.codigo}>
                      ...
                      {a.nombre}
                      <span className="ml-2 text-slate-400 text-xs">{formatCurrency(a.tarifa)}</span>
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 border-b">
                  <tr>
                    <th className="text-left px-2 py-1.5 text-xs font-semibold text-slate-500 uppercase">Servicio</th>
                  ...
                  {items.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50">
                      <td className="px-2 py-1.5">
                        ...
                        <p className="text-xs text-slate-400">{item.periodicidad === "M" ? "Mensual" : "Única vez"}</p>
                      </td>
                      <td className="px-2 py-1.5">
                        <Input
                      ...
                      <td className="px-2 py-1.5">
                        <Input
                          type="number" min="0" step="100" value={item.punitario}
                          onChange={(e) => actualizarItem(item.id, "punitario", Number(e.target.value))}
                          className="h-7 text-right text-sm w-28"
                        />
                      </td>
                      <td className="px-2 py-1.5">
                        <Input
                          type="number" min="0" max="100" step="1" value={item.descuento}
      ...
      <div className="flex justify-end gap-3">
        ...
        <Button
          ...
          disabled={guardando || !cliente || items.length === 0}
