...
import { createCliente, updateCliente, getNextInscripcion } from "@/db/queries/clientes";
...
export function ClienteFormulario({ modo, cliente, onGuardar, onCancelar }: Props) {
  ...
  const edad = fechaNac
    ? Math.floor((new Date().getTime() - new Date(fechaNac).getTime()) / (365.25 * 24 * 3600 * 1000))
    : null;

  // Obtener número de inscripción automático para nuevo cliente
  useEffect(() => {
    if (modo === "nuevo") {
      getNextInscripcion().then(setInscripcionAuto).catch(() => {});
    }
  }, [modo]);
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      if (modo === "nuevo") {
        ...
        success("Cliente creado", `${data.nombres} ${data.apellidos} registrado con inscripción #${inscripcionAuto}.`);
      } else if (cliente) {
      ...
      if (msg.includes("UNIQUE") || msg.includes("unique")) {
        error("Cédula duplicada", "La cédula ingresada ya está registrada. Por favor verifique los datos.");
      } else {
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-800">
            {modo === "nuevo" ? "Nuevo Cliente" : `Editar: ${cliente?.nombres} ${cliente?.apellidos}`}
          </h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="flex flex-col gap-4">
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-semibold text-slate-600">Foto del cliente</CardTitle>
              </CardHeader>
              <CardContent className="flex flex-col items-center gap-3">
                <div className="w-full aspect-[3/4] rounded-lg bg-slate-100 overflow-hidden relative flex items-center justify-center">
                  {fotoPreview ? (
                    ...
                    <div className="flex flex-col items-center gap-1 text-slate-400">
                      ...
                      <span className="text-xs text-center px-2 truncate w-full text-center">{fotoPath}</span>
                    </div>
                  ) : (
                ...
                {fotoPath && (
                  <Button
                    ...
                    onClick={() => { setFotoPath(null); setFotoPreview(null); }}
                  >
          ...
          <div className="lg:col-span-2 flex flex-col gap-4">
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-semibold text-slate-600">Identificación</CardTitle>
              </CardHeader>
              <CardContent className="grid grid-cols-2 gap-4">
                <FormField label="Cédula / Doc. Identidad" required error={errors.cedula?.message} htmlFor="cedula">
                  <Input
                    id="cedula"
                    {...register("cedula")}
                    ...
                    maxLength={15}
                    className={cn(errors.cedula && "border-destructive", modo === "editar" && "bg-slate-50")}
                  />
                </FormField>
                ...
                <FormField label="Nombres" required error={errors.nombres?.message} htmlFor="nombres">
                  <Input
                    id="nombres"
                    {...register("nombres", {
                ...
                <FormField label="Apellidos" required error={errors.apellidos?.message} htmlFor="apellidos">
                  <Input
                    id="apellidos"
                    {...register("apellidos", {
                      onChange: (e) => e.target.value = e.target.value.toUpperCase(),
                ...
                <FormField label="Fecha de nacimiento" required error={errors.fecha_nacimiento?.message} htmlFor="fecha_nacimiento">
                  <Input
                    id="fecha_nacimiento"
                    type="date"
                    {...register("fecha_nacimiento")}
                    className={cn(errors.fecha_nacimiento && "border-destructive")}
                  />
                </FormField>
            ...
            <Card>
              ...
              <CardContent className="grid grid-cols-2 gap-4">
                <FormField label="Dirección" required error={errors.direccion?.message} htmlFor="direccion" className="col-span-2">
                  <Input
                    id="direccion"
                    {...register("direccion")}
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <Button
                type="submit"
                disabled={guardando || (modo === "editar" && !isDirty && fotoPath === cliente?.foto_path)}
              >
                {guardando ? (
