      ...
      cell: ({ row }) => (
        <div>
          <p className="text-sm font-medium leading-tight">
            {row.original.nombre_actividad || row.original.id_actividad || '—'}
          </p>
          {row.original.periodicidad && (
            <p className="text-xs text-slate-400">
              {row.original.periodicidad === 'M' ? 'Mensual' : 'Única vez'}
            </p>
          )}
        </div>
      ),
    ...
    <div className="h-full overflow-y-auto">
      <div className="p-6 max-w-6xl mx-auto min-h-full">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800">Historial de Pagos</h1>
