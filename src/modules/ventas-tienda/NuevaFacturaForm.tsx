...
export function NuevaFacturaForm({ cedulaInicial, onGuardar, onCancelar }: Props) {
  ...
  const agregarItem = () => {
    ...
    if (!art) return;
    if (art.stock <= 0 && !permitirSinStock) {
      error('Sin stock', `${art.nombre} no tiene stock disponible.`);
      return;
    }
  ...
  const handleGuardar = async () => {
    if (!cliente) {
      error('Falta cliente', 'Seleccione un cliente.');
      return;
    }
    if (items.length === 0) {
      error('Sin ítems', 'Agregue al menos un artículo.');
      return;
    }
    if (!idFormaPago) {
      error('Forma de pago', 'Seleccione una forma de pago.');
      return;
    }
    ...
    if (!permitirSinStock) {
      for (const item of items) {
        if (item.cantidad > item.stockDisponible) {
          error(
            'Stock insuficiente',
            `${item.detalle}: solo hay ${item.stockDisponible} unidades disponibles.`
          );
          return;
        }
      }
    ...
    try {
      ...
      success(
        'Factura guardada',
        `Factura N° ${String(nroDocu).padStart(6, '0')} registrada. Stock actualizado.`
      );
      onGuardar();
    } catch (err) {
                    ...
                    {articulos.map((a) => (
                      <SelectItem key={a.codigo} value={a.codigo} disabled={a.stock <= 0}>
                        ...
                        <span className="ml-2 text-xs text-slate-400">
                          Stock: {a.stock} ·{' '}
                          {formatCurrency(a.precio_compra * (1 + a.ganancia / 100))}
                        </span>
                        {a.stock <= 0 && (
                          <span className="ml-1 text-xs text-red-400">(SIN STOCK)</span>
                        )}
                      </SelectItem>
                    ))}
                  </SelectContent>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {items.map((item) => (
                      <tr
                        ...
                        className={cn(
                          'hover:bg-slate-50',
                          item.cantidad > item.stockDisponible && 'bg-red-50'
                        )}
                      >
                        ...
                        <td className="px-2 py-1.5">
                          <Input
                            ...
                            value={item.impuesto}
                            onChange={(e) =>
                              actualizarItem(item.id, 'impuesto', Number(e.target.value))
                            }
                            className="h-7 text-right text-sm w-16"
                          />
                        </td>
                ...
                {esCredito && (
                  <div className="flex-1">
                    <FormField
                      label="Abono inicial ($)"
                      htmlFor="fac-abono"
                      description="0 = sin abono. El resto se generará como cuotas mensuales."
                    >
                      <Input
                        id="fac-abono"
                        type="number"
                  ...
                  {esCredito && abonoInicial > 0 && (
                    <div className="flex justify-between items-center text-amber-700">
                      ...
                      <span className="text-sm font-semibold tabular-nums">
                        {formatCurrency(total - abonoInicial)}
                      </span>
                    </div>
                  )}
