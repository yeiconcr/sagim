...
function FacturasLista({ refetchKey, onNuevo, onRefetch }: ListaProps) {
  ...
  const cargar = useCallback(async () => {
    setLoading(true);
    try {
      const result = await getFacturasTienda({
        pageSize: 200,
        estado: filtroEstado,
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-sm">
          {row.original.nombre_forma_pago || '—'}
          {row.original.plazo > 0 ? ` (${row.original.plazo}d)` : ''}
        </span>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2 justify-end">
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              setFacturaImprimir(row.original);
            }}
          >
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden p-6 gap-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card>
          <CardContent className="p-3 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-slate-500">Facturas del día</p>
      ...
      {loading ? (
        <PageLoading text="Cargando facturas..." />
      ) : (
        <DataTable
          columns={columns}
          data={facturas}
          searchPlaceholder="Buscar por cliente..."
          emptyMessage="No hay facturas para los filtros seleccionados."
          pageSize={20}
          showSearch={facturas.length > 10}
        />
      )}
      ...
      <ImprimirFacturaTienda factura={facturaImprimir} onClose={() => setFacturaImprimir(null)} />
    </div>
  );
}
