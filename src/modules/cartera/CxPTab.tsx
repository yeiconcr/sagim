...
export function CxPTab() {
  ...
  const handleAbonar = async (valorStr: string) => {
    ...
    if (isNaN(val) || val <= 0) {
      error('Valor inválido', 'El abono debe ser mayor a cero.');
      return;
    }
    setAbonando(true);
    try {
      ...
      cell: ({ getValue }) => (
        <span
          className={cn(
            ...
            getValue<number>() > 0 ? 'text-red-600' : 'text-green-600'
          )}
        >
          {formatCurrency(getValue<number>())}
        </span>
      ...
      cell: ({ row }) =>
        row.original.estado === 'P' ? (
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              setValorAbono(String(row.original.saldo));
              setAbonar(row.original);
            }}
          >
            Abonar
          </Button>
  ...
  return (
    <div className="flex flex-col gap-4 min-h-fit">
      ...
      {loading ? (
        <PageLoading text="Cargando cuentas por pagar..." />
      ) : (
        <DataTable
          columns={columns}
          data={cuentas}
