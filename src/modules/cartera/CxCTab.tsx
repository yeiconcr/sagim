...
import {
  getCtasPorCobrar,
  getSaldoCliente,
  registrarMovCtaCobrar,
  getCuotasByCliente,
  pagarCuota,
} from '@/db/queries/caja';
...
export function CxCTab() {
  ...
  const buscarCliente = async () => {
    ...
    try {
      ...
      const rows = await db.select<{ nombres: string; apellidos: string }[]>(
        'SELECT nombres, apellidos FROM clientes WHERE cedula = $1 LIMIT 1',
        [cedula]
      );
      if (rows.length === 0) {
  ...
  const handleRegistrarMov = async () => {
    ...
    if (!concepto.trim()) {
      error('Falta concepto', 'Ingrese una descripción.');
      return;
    }
    if (isNaN(val) || val <= 0) {
      error('Valor inválido', 'El importe debe ser mayor a cero.');
      return;
    }
    setGuardando(true);
    try {
      await registrarMovCtaCobrar({
        cedula,
        idTipomo: tipoMov,
        concepto,
        importe: val,
  ...
  const handlePagarCuota = async (valorStr: string) => {
    ...
    if (isNaN(val) || val <= 0) {
      error('Valor inválido', 'El valor de pago debe ser mayor a cero.');
      return;
    }
    setPagando(true);
    try {
  ...
  const colsMov: ColumnDef<MovCxC>[] = [
    {
      ...
      header: '#',
      size: 50,
      cell: ({ getValue }) => <span className="text-xs text-slate-400">{getValue<number>()}</span>,
    },
    {
      accessorKey: 'fecha_doc',
      header: 'Fecha',
      size: 100,
      cell: ({ getValue }) => <span className="text-sm">{formatDate(getValue<string>())}</span>,
    },
    {
      accessorKey: 'id_tipomo',
      header: 'Tipo',
      size: 110,
      cell: ({ getValue }) => (
        <span
          className={cn(
            ...
            TIPO_COLORS[getValue<string>()] ?? 'text-slate-600'
          )}
        >
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return (
          <span className={cn('text-sm tabular-nums', v > 0 && 'text-red-600 font-medium')}>
    ...
    {
      accessorKey: 'pago_clien',
      header: 'Abono',
      size: 110,
      cell: ({ getValue }) => {
        const v = getValue<number>();
        return (
          <span className={cn('text-sm tabular-nums', v > 0 && 'text-green-600 font-medium')}>
            {v > 0 ? formatCurrency(v) : '—'}
          </span>
        );
      },
  ...
  const colsCuotas: ColumnDef<Cuota>[] = [
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return (
          <Badge
            variant={(v[e] ?? 'outline') as 'success' | 'secondary' | 'destructive' | 'outline'}
            className="text-xs"
          >
            {e === 'P' ? 'PENDIENTE' : e === 'C' ? 'PAGADA' : 'VENCIDA'}
          </Badge>
        );
      },
    },
      ...
      cell: ({ row }) =>
        row.original.estado === 'P' ? (
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              setValorPago(String(row.original.importe_total - row.original.pagado));
              setConfirmPagar(row.original);
            }}
          >
            Pagar
          </Button>
  ...
  return (
    <div className="flex flex-col gap-4 min-h-fit">
        ...
        <Card className="border-blue-200 bg-blue-50/30">
          <CardContent className="p-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="col-span-4 flex justify-end gap-2">
              <Button variant="outline" size="sm" onClick={() => setMostrarMov(false)}>
      ...
      {loading ? (
        <PageLoading text="Cargando cartera..." />
      ) : nombreCliente ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 flex-1 min-h-0">
          {/* Movimientos CxC */}
          <div className="flex flex-col gap-2 min-h-0">
            <h3 className="font-semibold text-slate-700 text-sm flex-shrink-0">
      ...
      <InputDialog
        ...
        description={
          confirmPagar
            ? `Saldo pendiente: ${formatCurrency(confirmPagar.importe_total - confirmPagar.pagado)}`
            : ''
        }
