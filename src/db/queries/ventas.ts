...
export async function getRecibos(params: QueryParams = {}): Promise<PaginatedResult<Recibo>> {
  ...
  if (search) {
    conditions.push(
      `(r.cedula LIKE $${args.length + 1} OR c.nombres LIKE $${args.length + 2} OR c.apellidos LIKE $${args.length + 3})`
    );
  ...
  if (fechaDesde) {
    conditions.push(`r.fecha >= $${args.length + 1}`);
  ...
  const countRows = await dbSelect<{ c: number }>(
    `SELECT COUNT(*) as c FROM recibos r LEFT JOIN clientes c ON c.cedula = r.cedula ${where}`,
    args
  );
  ...
  const data = await dbSelect<Recibo>(
    ...
    LEFT JOIN forma_pago fp ON fp.id = r.id_forma_pago
    ${where}
...
export async function guardarReciboGym(params: {
  ...
  const ops = [
    // 1. Cabecera recibo
    {
      sql: `INSERT INTO recibos (nro_docu, fecha, hora, cedula, inscripcion, id_forma_pago, observaciones, valor_letras, estado)
...
export async function getFacturasTienda(
  params: QueryParams = {}
): Promise<PaginatedResult<FactuTienda>> {
  const { page = 1, pageSize = 50, search = '', fechaDesde, fechaHasta, estado = 'todos' } = params;
  const offset = (page - 1) * pageSize;
  const conditions: string[] = [];
  const args: unknown[] = [];
  ...
  if (estado !== 'todos') {
    conditions.push(`f.estado = $${args.length + 1}`);
    args.push(estado);
  }
  if (fechaDesde) {
    conditions.push(`f.fecha >= $${args.length + 1}`);
    args.push(fechaDesde);
  }
  if (fechaHasta) {
    conditions.push(`f.fecha <= $${args.length + 1}`);
  ...
  const countRows = await dbSelect<{ c: number }>(
    `SELECT COUNT(*) as c FROM factu_tienda f LEFT JOIN clientes c ON c.cedula = f.cedula ${where}`,
    args
  );
  const total = countRows[0]?.c ?? 0;
...
export async function guardarFacturaTienda(params: {
  ...
  if (params.plazo === 0) {
    ...
    ops.push({
      ...
      params: [
        ...
        params.total,
        params.total,
        `Venta Segun Factura No. ${nroDocu}`,
        params.usuario,
      ],
    });
  } else {
    ...
    if (params.abonoInicial > 0) {
      ops.push({
        ...
        params: [
          ...
          params.abonoInicial,
          params.abonoInicial,
          `Cuota Inicial a Factura No. ${nroDocu}`,
          params.usuario,
        ],
      });
    }
  ...
  // Fase 3: kardex + stock dentro de la misma transacción
  for (const item of params.items) {
    ops.push({
      sql: `INSERT INTO kardex (codigo_art, fecha, detalle, cantidad_sa, punitario_sa, total_sa)
            VALUES ($1,$2,$3,$4,$5,$6)`,
      params: [
        item.codigo,
        fechaHoy,
    ...
    ops.push({
      sql: `UPDATE inventario SET stock = stock - $1 WHERE codigo = $2`,
      params: [item.cantidad, item.codigo],
    });
  }
...
export async function anularFacturaTienda(
  ...
  // Revertir stock y kardex dentro de la transacción
  for (const item of det) {
    ops.push({
      sql: `INSERT INTO kardex (codigo_art, fecha, detalle, cantidad_in, punitario_in, total_in)
            VALUES ($1,$2,$3,$4,$5,$6)`,
      params: [
        item.codigo,
        fechaHoy,
...
export async function getConsultaPagosPorCliente(cedula: string): Promise<PagoCli[]> {
  const rows = await dbSelect<{ inscripcion: number }>(
    'SELECT inscripcion FROM clientes WHERE cedula = $1',
    [cedula]
  );
