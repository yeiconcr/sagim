...
export async function getCompras(params: QueryParams = {}): Promise<PaginatedResult<Compra>> {
  ...
  if (search) {
    conditions.push(
      `(p.nombre LIKE $${args.length + 1} OR c.nro_documento LIKE $${args.length + 2})`
    );
    args.push(`%${search}%`, `%${search}%`);
  }
  if (fechaDesde) {
    conditions.push(`c.fecha >= $${args.length + 1}`);
  ...
  const countRows = await dbSelect<{ c: number }>(
    `SELECT COUNT(*) as c FROM compras c LEFT JOIN proveedores p ON p.id = c.id_proveedor ${where}`,
    args
  );
...
export async function getDetCompraPorNro(nroCompra: number): Promise<DetCompra[]> {
  return dbSelect<DetCompra>('SELECT * FROM det_compra WHERE nro_compra = $1 ORDER BY id ASC', [
    nroCompra,
  ]);
}

export async function getNextNroCompra(): Promise<number> {
  const maxRow = await dbSelectOne<{ max: number | null }>(
    'SELECT MAX(nro_compra) as max FROM compras'
  );
...
export async function guardarCompra(params: {
  ...
  if (params.plazo === 0) {
    ...
    const nombreProv =
      provResult.length > 0 ? provResult[0].nombre : `Proveedor #${params.idProveedor}`;
  ...
  // Kardex + stock + actualización precio dentro de la transacción
  for (const item of params.items) {
    if (item.codigo && item.codigo !== 'GASTO') {
      ...
      ops.push({
        sql: `UPDATE inventario SET stock = stock + $1, precio_compra = $2 WHERE codigo = $3`,
        params: [item.cantidad, item.punitario, item.codigo],
      });
    }
  }
...
export async function abonarCtaPorPagar(
  id: number,
  valorAbono: number,
  usuario: string
): Promise<void> {
  const cpRow = await dbSelectOne<{ saldo: number; id_proveedor: number }>(
    'SELECT saldo, id_proveedor FROM ctas_por_pagar WHERE id = $1',
    [id]
  );
