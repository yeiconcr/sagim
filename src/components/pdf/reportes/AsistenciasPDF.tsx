...
interface AsistenciasPDFProps {
  ...
  subtitulo?: string;
  generadoPor?: string;
  headers: Array<{ label: string; key: string; width?: number; align?: "left" | "center" | "right" }>;
  ...
  return (
    <Document>
      <Page size="A4" style={[styles.page, { paddingHorizontal: 20 }]} orientation="portrait">
        ...
        {/* Estadísticas */}
        {p.estadisticas && (
          <View style={{ flexDirection: "row", justifyContent: "space-around", marginBottom: 15, padding: 10, backgroundColor: "#f8fafc", borderRadius: 4 }}>
            <View style={{ alignItems: "center" }}>
              <Text style={{ fontSize: 16, fontWeight: "bold", color: "#1e40af" }}>{p.estadisticas.totalEntradas}</Text>
              <Text style={{ fontSize: 8, color: "#64748b" }}>Total Entradas</Text>
            </View>
            <View style={{ alignItems: "center" }}>
              <Text style={{ fontSize: 16, fontWeight: "bold", color: "#059669" }}>{p.estadisticas.clientesUnicos}</Text>
              <Text style={{ fontSize: 8, color: "#64748b" }}>Clientes Únicos</Text>
            </View>
            <View style={{ alignItems: "center" }}>
              <Text style={{ fontSize: 16, fontWeight: "bold", color: "#7c3aed" }}>{p.estadisticas.promedioDiario}</Text>
              <Text style={{ fontSize: 8, color: "#64748b" }}>Promedio Diario</Text>
            </View>
            {p.estadisticas.horaPico && (
              <View style={{ alignItems: "center" }}>
                <Text style={{ fontSize: 16, fontWeight: "bold", color: "#ea580c" }}>{p.estadisticas.horaPico}</Text>
