  ...
  return (
    <Document>
      <Page size={[RECEIPT_WIDTH, 600]} style={tirillaStyles.page}>
        ...
        <Text style={[tirillaStyles.centerText, tirillaStyles.bold, { fontSize: 10 }]}>RECIBO DE CAJA</Text>
        <Text style={tirillaStyles.centerText}>Nro: {String(p.nroDocu).padStart(6, "0")}</Text>
        <Text style={tirillaStyles.centerText}>
          Fecha: {formatDate(p.fecha)} {p.hora ? new Date(p.hora).toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' }) : ""}
        </Text>
        
        {anulado && (
          <Text style={[tirillaStyles.centerText, tirillaStyles.bold, { marginTop: 4, fontSize: 12 }]}>( ANULADO )</Text>
        )}
        ...
        {p.items.map((item, i) => (
          <View key={i} style={tirillaStyles.rowItem}>
            ...
            <View style={tirillaStyles.itemSub}>
              <Text>{item.cantidad} x {formatCurrency(item.punitario)}</Text>
