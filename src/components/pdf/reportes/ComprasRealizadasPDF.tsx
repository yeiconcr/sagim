...
// Reporte: ComprasRealizadasPDF
// Placeholder funcional — los datos se inyectan desde el módulo Reportes
export function ComprasRealizadasPDF(p: any) {
  const rows = p.data ?? [];
  const headers = p.headers ?? [];
  return (
    <Document>
      <Page size="A4" style={[styles.page, { paddingHorizontal: 20 }]} orientation={p.orientation ?? "portrait"}>
        <ReporteHeader
          gimnasio={p.gimnasio ?? "SAGIM"} nit={p.nit} direccion={p.direccion} telefono={p.telefono}
          titulo={p.titulo ?? "ComprasRealizadasPDF"}
