    ...
    <Document>
      <Page size="A4" style={styles.page}>
        <ReporteHeader
          ...
          subtitulo={`No. ${String(p.nroDocu).padStart(6, '0')}`}
        />

        {anulada && (
          <View
            style={{ backgroundColor: '#fee2e2', borderRadius: 4, padding: 8, marginBottom: 12 }}
          >
            <Text
              style={{
                fontSize: 16,
          ...
          <View style={{ flexDirection: 'row', gap: 16 }}>
            <View style={{ flex: 1 }}>
              <View style={styles.dataRow}>
                ...
                <Text style={styles.dataValue}>{p.cliente}</Text>
              </View>
              <View style={styles.dataRow}>
            ...
            <View style={{ flex: 1 }}>
              <View style={styles.dataRow}>
                ...
                <Text style={styles.dataValue}>{formatDate(p.fecha)}</Text>
              </View>
              <View style={styles.dataRow}>
                <Text style={styles.dataLabel}>Forma de pago:</Text>
                <Text style={styles.dataValue}>
                  {p.formaPago}
                  {p.plazo > 0 ? ` (${p.plazo} días)` : ''}
                </Text>
              </View>
            </View>
          </View>
        ...
        <View style={styles.table}>
          ...
          {p.items.map((item, i) => (
            <View key={i} style={[styles.tableRow, i % 2 === 1 ? styles.tableRowAlt : {}]}>
              ...
              <Text style={[styles.tableCell, styles.tableCellRight, { width: 35 }]}>
                {item.cantidad}
              </Text>
              <Text style={[styles.tableCell, styles.tableCellRight, { width: 70 }]}>
                {formatCurrency(item.punitario)}
              </Text>
              <Text style={[styles.tableCell, styles.tableCellRight, { width: 25 }]}>
                {item.descuento > 0 ? `${item.descuento}%` : '—'}
              </Text>
              <Text style={[styles.tableCell, styles.tableCellRight, { width: 25 }]}>
                {item.impuesto > 0 ? `${item.impuesto}%` : '—'}
              </Text>
              <Text
                style={[
                  styles.tableCell,
        ...
        <View style={styles.totalesSection}>
          <View>
            <View style={styles.totalRow}>
              ...
              <Text style={styles.totalValor}>{formatCurrency(p.subtotal)}</Text>
            </View>
            <View style={styles.totalRow}>
              <Text style={styles.totalLabel}>IVA:</Text>
              <Text style={styles.totalValor}>{formatCurrency(p.iva)}</Text>
            </View>
            <View
              style={[
                styles.totalRow,
                { borderTopWidth: 0.5, borderTopColor: COLORS.border, paddingTop: 3 },
              ]}
            >
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginTop: 40 }}>
          <View style={{ alignItems: 'center' }}>
            <View style={{ width: 120, borderBottomWidth: 0.5, borderBottomColor: COLORS.text }} />
            <Text style={{ fontSize: 8, color: COLORS.secondary, marginTop: 4 }}>
              Firma del Cliente
            </Text>
          </View>
          <View style={{ alignItems: 'center' }}>
            <View style={{ width: 120, borderBottomWidth: 0.5, borderBottomColor: COLORS.text }} />
            <Text style={{ fontSize: 8, color: COLORS.secondary, marginTop: 4 }}>Recibido por</Text>
          </View>
        </View>
