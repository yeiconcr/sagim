...
interface ClienteRow { inscripcion: number; cedula: string; nombres: string; apellidos: string; celular?: string | null; ciudad?: string | null; fecha_inscripcion?: string | null; estado: string; }

interface Props {
  gimnasio: string; nit?: string | null; direccion?: string | null; telefono?: string | null;
  clientes: ClienteRow[]; filtro?: string; generadoPor?: string;
}
    ...
    <Document>
      <Page size="A4" style={[styles.page, { paddingHorizontal: 20 }]} orientation="landscape">
        <ReporteHeader gimnasio={p.gimnasio} nit={p.nit} direccion={p.direccion} telefono={p.telefono}
          titulo="LISTADO DE CLIENTES" subtitulo={`Estado: ${p.filtro || "Todos"} · Total: ${p.clientes.length}`} />
        <ReporteTable
          headers={[
        ...
        <View style={{ marginTop: 8, flexDirection: "row", justifyContent: "flex-end" }}>
          <Text style={{ fontSize: 8, color: COLORS.secondary }}>Total registros: <Text style={{ fontFamily: "Helvetica-Bold" }}>{p.clientes.length}</Text></Text>
        </View>
