...
interface KRow { fecha: string; detalle?: string | null; cantidad_in: number; punitario_in: number; total_in: number; cantidad_sa: number; punitario_sa: number; total_sa: number; saldo?: number; }
interface Props { gimnasio: string; nit?: string | null; direccion?: string | null; telefono?: string | null; articulo: string; codigo: string; stockActual: number; kardex: KRow[]; fechaDesde?: string; fechaHasta?: string; generadoPor?: string; }
      ...
      <Page size="A4" style={[styles.page, { paddingHorizontal: 20 }]} orientation="landscape">
        ...
        <ReporteTable
          ...
          getCell={(row, key) => {
            ...
            if (key === "total_in") { const v = row.total_in as number; return v > 0 ? formatCurrency(v) : "—"; }
            if (key === "total_sa") { const v = row.total_sa as number; return v > 0 ? formatCurrency(v) : "—"; }
            if (key === "cantidad_in") { const v = row.cantidad_in as number; return v > 0 ? `+${v}` : "—"; }
            if (key === "cantidad_sa") { const v = row.cantidad_sa as number; return v > 0 ? `-${v}` : "—"; }
